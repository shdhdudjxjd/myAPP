...
def EN_CN(manager,sql):
   EN_CN=manager.getEN_CN()
   Record=EN_CN
   word=Record['word']
   ...
   with col1:
        ...
        with st.container(border=True):
            ...
            with col_1:
                ...
                args = (manager, sql, manager.getUser(), manager.nowName(), nowID, 3, is_correct)
            ...
            with col_2:
                st.button('下一个',icon='➡️',use_container_width=True,on_click=addEN_CN_ID,args=(manager,))
            if is_subimt:
                if Mychoice != answer:
                    st.error("❌ 回答错误")
